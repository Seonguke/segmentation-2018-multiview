...
//   }
    ...
    /* Input parameters */
    double regions_i, regions_j, total_regions, variables;
    
    /* Input parameters */
    regions_i = mxGetScalar(prhs[0]);
    regions_j = mxGetScalar(prhs[1]);
    variables = mxGetScalar(prhs[2]);
    
    ConstMatlabMultiArray<double> regions_adjacency(prhs[3]);
