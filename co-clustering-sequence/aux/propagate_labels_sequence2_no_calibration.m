function [ partitions_vector_out, images_vector_out, last_label ] = propagate_labels_sequence2_no_calibration( coarse_partition_i, coarse_partition_j, I_i, I_j, I_new, fine_partition_j, fine_partition_new, ucm_j, ucm_new, merging_sequence_j, merging_sequence_new, vectors_i, vectors_j, vectors_new, window_similarities, w_color, w_hog, w_sift, pixel_based_MV, hierarchical_constraint_MV, hierarchical_constraint_opt, motion_estimation, num_clusters, last_label )
...
%tic;
[ A, b ] = inequalities_sparse2(double(cumulate(end)), double(variables_total), double(regions_adjacency_total_offset), double(regions_variables_offset), double(partitions_adjacency_total) );
...
%% Conditions on number of clusters
edge_ids = select_edges_from_hierarchy_sequence( B_vector(2).merging_sequence, B_vector(2).RAG, region_partitions(2), B_vector(3).merging_sequence, B_vector(3).RAG, region_partitions(3), regions_adjacency_total, partitions_adjacency_total);
Aeq2 = zeros(1,length(partitions_adjacency_total));
Aeq2(edge_ids) = 1;
Aeq = [Aeq;Aeq2];
beq = [beq num_clusters-2];

% edge_ids = select_edges_from_hierarchy_sequence_new( B_vector(3).merging_sequence, B_vector(3).RAG, region_partitions(3), regions_adjacency_total, partitions_adjacency_total);
% Aeq2 = zeros(1,length(partitions_adjacency_total));
% Aeq2(edge_ids) = 1;
% Aeq = [Aeq;Aeq2];
% beq = [beq num_clusters-2-length(unique(coarse_partition_j))];
