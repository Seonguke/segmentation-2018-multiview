function [ edge_ids ] = select_edges_from_hierarchy_sequence( merging_sequence_i, RAG_i, regions_i, merging_sequence_j, RAG_j, regions_j, regions_adjacency_total, partitions_adjacency_total)
...
%   Detailed explanation goes here
    ...
    for ii=1:size(merging_sequence_i,1)
        ...
        col_subset = find(partitions_adjacency_total(1,:)==2 & partitions_adjacency_total(2,:)==2);
        offset = col_subset(1)-1;
        regions_adjacency = regions_adjacency_total(:,col_subset);
        col_subset = find(regions_adjacency(1,:)==label1);
        edge_id = col_subset(find(regions_adjacency(2,col_subset)==label2)) + offset;
    ...
    for ii=1:size(merging_sequence_j,1)
        ...
        %find label1-label2 pair in region_adjacency
        col_subset = find(partitions_adjacency_total(1,:)==3 & partitions_adjacency_total(2,:)==3);
        offset = col_subset(1)-1;
        regions_adjacency = regions_adjacency_total(:,col_subset);
        col_subset = find(regions_adjacency(1,:)==label1);
        edge_id = col_subset(find(regions_adjacency(2,col_subset)==label2)) + offset;
