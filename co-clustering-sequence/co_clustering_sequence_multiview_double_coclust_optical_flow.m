function co_clustering_sequence_multiview_double_coclust_optical_flow(sequence_name, out_path, num_regions)
...
if ~exist(path_ima)
    ...
    [partition_i_inter, partition_j_inter, image_i_inter, image_j_inter, partition_i_intra, partition_j_intra] = ...
        co_clustering_global_hierarchy_optical_flow_num_clusters( sequence_name, 2, I_i, I_j, seg_i, seg_j, ucm_i, ucm_j, merging_sequence_i, merging_sequence_j, vectors_i, vectors_j, window, w_color, w_hog, w_sift, hierarchical_constraint_opt, 2*num_regions  );
    
    lambda_inter = 1;
    [partition_i_inter, partition_j_inter, image_i_inter, image_j_inter, partition_i_intra, partition_j_intra] = ...
        co_clustering_global_hierarchy_optical_flow( sequence_name, 2, I_i, I_j, partition_i_intra, partition_j_intra, ucm_i, ucm_j, [], [], vectors_i, vectors_j, lambda_inter, window, w_color, w_hog, w_sift, false);

    label1 = max(max(partition_i_inter));
    label2 = max(max(partition_j_inter));
    last_label=max([label1 label2]);
    
    partitions_vector_total_intra(:,:,1) = partition_i_intra;
    partitions_vector_total_intra(:,:,2) = partition_j_intra;
    ...
    for ii=3:numel(frames)
        ...
        [ partitions_vector_out_intra] = propagate_labels_sequence2_intra_optical_flow( sequence_name, ii, partition_i_inter, partition_j_inter, I_i, I_j, I_new, seg_j, seg_new, ucm_j, ucm_new, merging_sequence_j, merging_sequence_new, vectors_i, vectors_j, vectors_new, window, w_color, w_hog, w_sift, hierarchical_constraint_opt, 2*num_regions);

        if ~isempty(partitions_vector_out_intra)

            [ partitions_vector_out, images_vector_out, last_label ] = propagate_labels_sequence2_inter_optical_flow( sequence_name, ii, partition_i_inter, partition_j_inter, I_i, I_j, I_new, partition_j_intra, partitions_vector_out_intra(:,:,3), ucm_j, ucm_new, [], [], vectors_i, vectors_j, vectors_new, lambda_inter, window, w_color, w_hog, w_sift, false, last_label);

            if ~isempty(partitions_vector_out)
                partitions_vector_total(:,:,ii) = partitions_vector_out(:,:,3);
                partitions_vector_total_intra(:,:,ii) = partitions_vector_out_intra(:,:,3);
                ...
                partition_j_intra = partitions_vector_out_intra(:,:,3);
            else
                break
            end
        else
    ...
    if ii == numel(frames)
        save(path_ima, 'partitions_vector_total','partitions_vector_total_intra');
