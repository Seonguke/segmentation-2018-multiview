function [ucm, ws_a] = contours2ucm_normal_vectors(pb_oriented, fmt, fine_partition)
...
if nargin<3
    [ws_wt, ws_a] = create_finest_partition(pb_oriented);
else
    [ws_wt, ws_a] = create_finest_partition(pb_oriented, fine_partition);
end
...
if nargin<3
    ...
    ws_wt2 = double(super_contour_4c(ws_wt, fine_partition));
end
...
function [ws_wt, ws_a] = create_finest_partition(pb_oriented, fine_partition)

pb = max(pb_oriented,[],3);

if nargin==1
    fine_partition = watershed(pb);
    ws_bw = (fine_partition == 0);
else
    % Extract boundaries
    ws_bw = (seg2bmap(fine_partition) == 1);
end
...
for e = 1 : numel(contours.edge_x_coords)
    ...
    for p = 1 : numel(contours.edge_x_coords{e}),
        ...
        ws_a(contours.edge_x_coords{e}(p), contours.edge_y_coords{e}(p)) = orient(e); %ADDED
    end
    v1=contours.vertices(contours.edges(e,1),:);
    v2=contours.vertices(contours.edges(e,2),:);
...
function [pb2, V, H] = super_contour_4c(pb, fine_partition)
...
if nargin>1
    bmap2 = seg2bmap2(fine_partition);
    pb2 = pb2(1:end-1,1:end-1).*bmap2(3:end, 3:end);
    pb2(end+1,:) = pb2(end,:);
    pb2(:,end+1) = pb2(:,end);
end
%%
...
for r = 1 : numel(R),
    ...
    elseif(yc==2 && xc==size(ws_clean,1)-1)
        vec = max(ws_clean(xc-2, yc+1), ws_clean(xc-1, yc+2));
        if ws_clean(xc-2, yc+1) < ws_clean(xc-1, yc+2), 
           ws_clean(xc, yc+1) = 0;
           ws_clean(xc-1, yc) = vec(1);
        ...
        ws_clean(xc-1, yc+1) = vec(1);
    elseif(yc==size(ws_clean,2)-1 && xc==size(ws_clean,1)-1)
        vec = max(ws_clean(xc-2, yc-1), ws_clean(xc-1, yc-2));
        ...
        ws_clean(xc-1, yc-1) = vec(1);
    elseif(yc==size(ws_clean,2)-1 && xc==2)
        vec = max(ws_clean(xc+2, yc-1), ws_clean(xc+1, yc-2));
