...
{
    if(nrhs != 8)
        mexErrMsgTxt("There should be 8 inputs (labeled_descriptors1, labeled_descriptors2, labeled_elements1, labeled_elements2, Sigma, window, MV_x, MV_y)");
...
//     double *accW;
     ...
     * prhs[5] -> window                  (int)
     * prhs[6] -> MV_x   (double(matrix))
     * prhs[7] -> MV_y (double(matrix))
     */
...
//     sigma = mxGetPr(prhs[4]);
    ...
    ConstMatlabMultiArray<double> MV_x_mat(prhs[6]);
    ConstMatlabMultiArray<double> MV_y_mat(prhs[7]);
...
//                 mxArray *motion_estimation_v;
                
                int col_offset = 2*round(MV_x_mat[ii/2][jj/2]);
                int row_offset = 2*round(MV_y_mat[ii/2][jj/2]);
                ...
                for(int mm = -window; mm <= window; mm++){
                    for(int nn = -window; nn <= window; nn++){
                        ...
                        if((jj+nn+col_offset >= 0) && (jj+nn+col_offset < n_le) && (ii+mm+row_offset >= 0) && (ii+mm+row_offset < m_le)){ 

                            if(labeled_elements1[ii+mm+row_offset][jj+nn+col_offset] != 0){
                                ...
                                if(dist <= window){
                                    ...
                                    for(int kk = 0; kk < m_ld; kk++){
                                        ...
                                        accum = accum + (Sigma[kk][kk] *
                                                (labeled_descriptors2[kk][(int)labeled_elements2[ii][jj]-1] - labeled_descriptors1[kk][(int)labeled_elements1[ii+mm+row_offset][jj+nn+col_offset]-1])*
                                                (labeled_descriptors2[kk][(int)labeled_elements2[ii][jj]-1] - labeled_descriptors1[kk][(int)labeled_elements1[ii+mm+row_offset][jj+nn+col_offset]-1]));
                                    ...
                                    m_W.push_back(labeled_elements1[ii+mm+row_offset][jj+nn+col_offset]-1);
...
//     mxDestroyArray(idx_neighbors_min_pr);
//     mxDestroyArray(idx_neighbors_max_pr);
// 	mxDestroyArray(motion_estimation_v);
