...
{
    if(nrhs != 4)
        mexErrMsgTxt("There should be 4 inputs (partition1, partition2, MV_x and MV_y)");
     ...
     * prhs[2] -> motion_estimation_vectors     (double(matrix))
     */
     
    partition1 = mxGetPr(prhs[0]);
    partition2 = mxGetPr(prhs[1]);
    int numelems = mxGetNumberOfElements(prhs[2]); 
...
//     MatlabMultiArray<double> partition2_data(partition2_data_pr);
    ...
    double *MV_x_mat;
    MV_x_mat = mxGetPr(prhs[2]);
    
    double *MV_y_mat;
    MV_y_mat = mxGetPr(prhs[3]);
    
    // INTER
    
    for(int i = 0; i < m; i++){
        for(int j = 0; j < n; j++){
            int region_label_2 = partition2[i + j*m];
            double MV_x = MV_x_mat[i + j*m];
            double MV_y = MV_y_mat[i + j*m];

            int new_col_pos = j + round(MV_x);
            int new_row_pos = i + round(MV_y);
            if (new_col_pos < 0)
                new_col_pos = 0;
            if (new_row_pos < 0)
                new_row_pos = 0;
            ...
            RAG_Inter[partition1[new_row_pos + new_col_pos*m]-1][partition2[i + j*m]-1] = 1;
        }
    }
    
//     mxDestroyArray(partition1_data_pr);
//     mxDestroyArray(partition2_data_pr);
