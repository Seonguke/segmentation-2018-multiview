function co_clustering_sequence_multiview_double_coclustering_no_calib(sequence_name, out_path, num_regions)
...
if ~exist(path_ima)
    ...
    lambda_inter = 1;
    [partition_i_inter, partition_j_inter, image_i_inter, image_j_inter, partition_i_intra, partition_j_intra] = ...
        co_clustering_global_hierarchy_no_calibration( I_i, I_j, partition_i_intra, partition_j_intra, ucm_i, ucm_j, [], [], vectors_i, vectors_j, lambda_inter, window, w_color, w_hog, w_sift, pixel_based_MV, false, false, motion_estimation );
    ...
    for ii=3:numel(frames)
        ...
        [ partitions_vector_out_intra] = propagate_labels_sequence2_intra_no_calib( partition_i_inter, partition_j_inter, I_i, I_j, I_new, seg_j, seg_new, ucm_j, ucm_new, merging_sequence_j, merging_sequence_new, vectors_i, vectors_j, vectors_new, window, w_color, w_hog, w_sift, pixel_based_MV, hierarchical_constraint_MV, hierarchical_constraint_opt, motion_estimation, 2*num_regions);
        
        if ~isempty(partitions_vector_out_intra)
        
            [ partitions_vector_out, images_vector_out, last_label ] = propagate_labels_sequence2_inter_no_calib( partition_i_inter, partition_j_inter, I_i, I_j, I_new, partition_j_intra, partitions_vector_out_intra(:,:,3), ucm_j, ucm_new, [], [], vectors_i, vectors_j, vectors_new, lambda_inter, window, w_color, w_hog, w_sift, pixel_based_MV, false, false, motion_estimation, last_label);
