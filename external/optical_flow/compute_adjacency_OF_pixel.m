function [ adjacency_matrix ] = compute_adjacency_OF_pixel( seg_i, seg_j, OF_regions_x, OF_regions_y, object_mask, flow )
...
delta = uint8(round(flow));
for i=1:s_x
    for j=1:s_y        
        %% Compute adjacent area with OF       
        if((i+delta(i,j,2))>0 && (i+delta(i,j,2))<=s_y && (j+delta(i,j,1))>0 && (j+delta(i,j,1))<=s_x)
            adjacency_matrix_with_OF(seg_i(i,j),seg_j(i+delta(i,j,2),j+delta(i,j,1))) = adjacency_matrix_with_OF(seg_i(i,j),seg_j(i+delta(i,j,2),j+delta(i,j,1))) + 1;           
        end      
...
for j=1:regions_j
    ...
    for i=1:regions_i
        if(regions_mask(i) == 1)
            if(adjacency_matrix_with_OF(i,j)/area_regions_i(i) > 0.0)
    ...
    if(total_area_adjacency/area_regions_j(j) > 0.0)
