function [partition_i_inter, partition_j_inter, image_i_inter, image_j_inter, partition_i_intra, partition_j_intra] = ...
    co_clustering_global_hierarchy_no_calibration( I_i, I_j, seg_i, seg_j, ucm_i, ucm_j, merging_sequence_i, merging_sequence_j, vectors_i, vectors_j, lambda, window_similarities, w_color, w_hog, w_sift, pixel_based_MV, hierarchical_constraint_MV, hierarchical_constraint_opt, motion_estimation  )
...
if motion_estimation
    [f_sift_i, d_sift_i] = vl_sift(im2single(rgb2gray(I_i)));
    [f_sift_j, d_sift_j] = vl_sift(im2single(rgb2gray(I_j)));
...
%     matches = filter_matches(f_sift_j,f_sift_i,matches);
    ...
    if pixel_based_MV
        if hierarchical_constraint_MV
            [idx_mat,estimated_MVs] = sift_based_motion_estimation_mat_hierarchical(seg_j,merging_sequence_j,f_sift_j,f_sift_i,matches);
        else
            [idx_mat,estimated_MVs] = sift_based_motion_estimation_mat(seg_j,f_sift_j,f_sift_i,matches);
        end
        RAG_ij = RAG_Inter_motion_estimation_mex_pixel_based(double(seg_i), double(seg_j), idx_mat, estimated_MVs);
    else
        if hierarchical_constraint_MV
            motion_estimation_vectors = sift_based_motion_estimation_1vector_hierarchical(seg_j,regions_j,merging_sequence_j,f_sift_j,f_sift_i,matches);
        else
            motion_estimation_vectors = sift_based_motion_estimation_1vector(seg_j,regions_j,f_sift_j,f_sift_i,matches);
        end
        RAG_ij = RAG_Inter_motion_estimation_mex(double(seg_i), double(seg_j), motion_estimation_vectors);
    end
else
    RAG_ij = RAG_Inter_mex(double(seg_i), double(seg_j));
end
...
if motion_estimation
    if pixel_based_MV
        W = computeWmatrix_sparse_motion_estimation_pixel_based_mex(d_i,d_j,labeled_elements_i,labeled_elements_j,Sigma,window_similarities, idx_mat, estimated_MVs);
    else
        W = computeWmatrix_sparse_motion_estimation_mex(d_i,d_j,labeled_elements_i,labeled_elements_j,Sigma,window_similarities, idx_neighbors_j, motion_estimation_vectors);
    %         W = computeWmatrix_sparse_motion_estimation_without_descriptors_mex(d_i,d_j,labeled_elements_i,labeled_elements_j,window_similarities, idx_neighbors_j, motion_estimation_vectors);
    end
else
    W = computeWmatrix_sparse_mex(d_i,d_j,labeled_elements_i,labeled_elements_j,Sigma,window_similarities);
end
...
Q= [lambda*Q11-1 Q12-1; Q12'-1 lambda*Q22-1];
